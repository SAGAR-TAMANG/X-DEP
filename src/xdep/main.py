...
def get_training_configurations():
    ...
    # Model Selection (This part is fine)
...
def build_and_train_model(config, num_classes, train_ds, val_ds, test_ds, output_dir):
    ...
    report = classification_report(y_true, y_pred, target_names=test_ds.class_names, output_dict=True, zero_division=0)
    results = {
    ...
    if 'matrix' in config['visualizations']:
        generate_confusion_matrix(y_true, y_pred, test_ds.class_names, model_name, output_dir)
...
def run_xai_diagnostics(model, model_name, test_ds, num_samples, class_names, output_dir):
    ...
    #     print("Could not find a convolutional layer for Grad-CAM. Skipping.")
    #     return
    # for i in range(num_samples):
...
def save_results_to_csv(all_results, output_dir):
    ...
    fieldnames = [
        ...
        'training_type', 'step1_epochs', 'step1_lr', 'step2_epochs',
        'step2_lr', 'unfreeze_layers', 'patience'
    ]
