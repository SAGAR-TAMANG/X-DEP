...
from tensorflow.keras.applications import vgg16, resnet50, densenet, mobilenet_v2
...
IMG_SIZE = (224, 224)
MODEL_OPTIONS = {
    "1": {"name": "VGG16", "func": VGG16, "preprocess": vgg16.preprocess_input},
    "2": {"name": "ResNet50", "func": ResNet50, "preprocess": resnet50.preprocess_input},
    "3": {"name": "DenseNet121", "func": DenseNet121, "preprocess": densenet.preprocess_input},
    "4": {"name": "MobileNetV2", "func": MobileNetV2, "preprocess": mobilenet_v2.preprocess_input}
}
...
def build_and_train_model(config, num_classes, train_ds, val_ds, test_ds, output_dir):
    ...
    preprocess_input = model_info['preprocess']

    inputs = Input(shape=(*IMG_SIZE, 3))
    x = preprocess_input(inputs)  # Apply model-specific preprocessing here
    x = base_model(x, training=False) # Pass preprocessed data to the base model
    x = Flatten()(x)
...
def get_gradcam_heatmap(img_array, model, base_model_name, last_conv_layer_name, pred_index=None):
    """Creates a Grad-CAM heatmap."""
    
    # --- CHANGE THE MODEL CREATION ---
    # Access the layer inside the nested base model
    grad_model = Model(
...
def run_xai_diagnostics(model, model_name, test_ds, num_samples, class_names, output_dir):
    ...
    try:
        # We already have the base_model object here
    ...
    for i in range(num_samples):
        ...
        # Pass the base model's name (which is base_model.name)
        heatmap = get_gradcam_heatmap(img_array, model, base_model.name, last_conv_layer_name)
...
def save_results_to_csv(all_results, output_dir):
    ...
    fieldnames = [
        ...
        'training_type', 'step1_epochs', 'step1_lr', 'step1_lr_scheduler',
        'step2_epochs', 'step2_lr', 'step2_lr_scheduler', 
        'unfreeze_layers', 'patience'
    ]
    ...
    print(f"\n--- Final Report ---")
    console.print(f"All results have been saved to [bold green]{csv_path}[/bold green]")
